...
func TestChatStore(t *testing.T) {
	...
	for _, m := range []string{"a: hi", "b: yo"} {
		if err := s.save(context.Background(), "general", []byte(m)); err != nil {
			t.Fatalf("save: %v", err)
		}
	}
	if err := s.save(context.Background(), "music", []byte("c: track")); err != nil {
		t.Fatalf("save: %v", err)
	}

	t.Run("recent returns a room's messages oldest-first", func(t *testing.T) {
		got, err := s.recent(context.Background(), "general", 10)
		if err != nil {
	...
	t.Run("recent is scoped per room", func(t *testing.T) {
		if got, _ := s.recent(context.Background(), "music", 10); len(got) != 1 {
			t.Fatalf("recent(music) len = %d, want 1", len(got))
		}
		if got, _ := s.recent(context.Background(), "art", 10); len(got) != 0 {
			t.Fatalf("recent(art) len = %d, want 0", len(got))
		}
	})

	t.Run("recent honors the limit (keeping the newest)", func(t *testing.T) {
		got, _ := s.recent(context.Background(), "general", 1)
	...
	t.Run("purge keeps recent, deletes past the window", func(t *testing.T) {
		if n, err := s.purgeOlderThan(context.Background(), 24*time.Hour); err != nil || n != 0 {
			t.Fatalf("purge(24h) = %d (err %v), want 0", n, err)
		}
		// A negative window puts the cutoff in the future, so everything is old.
		if n, err := s.purgeOlderThan(context.Background(), -time.Hour); err != nil || n != 3 {
			t.Fatalf("purge(-1h) = %d (err %v), want 3", n, err)
		}
		if got, _ := s.recent(context.Background(), "general", 10); len(got) != 0 {
