...
- **Images**: PNG/GIF assets ship with a lossless **WebP** variant generated at
  ...
  original. No encoding happens at startup.
...
The TypeScript is compiled into `site/static/js` and the WebP image variants are
generated by `cmd/genwebp`, both in the Docker build before `go build`, so
